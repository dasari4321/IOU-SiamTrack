...
from pysot.models.block_a1 import get_subwindow ########################
...
#            raise Exception('size not match!')
    ...
    # calculate z crop size
        ...
        ####$$$$$$$$$$$$$$$$$
        gt_patche = get_subwindow(img, center_pos,
    ...
    def __getitem__(self, index):
        ...
        # augmentation
        template, bbox_prev = self.template_aug(template_image,
                                        template_box,
        ...
        gt_patches = self._gt_patch(search_image,bbox) 
        img_shape = template_image.shape
        return {
                'template': template,
                'search': search,
                'label_cls': cls,
                ...
                'bbox': np.array(bbox),
                'bbox_prev':np.array(bbox_prev),
                'T': template_image,
                'S': search_image,
                'bb1':np.array(template_box),
                'bb2':np.array(search_box),
...
#    D=TrkDataset()
#    train_loader = DataLoader(D, batch_size=8, num_workers=8, pin_memory=True)
#    model = ModelBuilder().cuda().train()
#    for idx, data in enumerate(train_loader):
#        outputs, cls2, loc = model(data)
