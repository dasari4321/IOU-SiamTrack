...
def fn(imgs,bboxs,clss,locs): 
    ...
    for img,bbox_gt,cls,loc in zip(imgs,bboxs,clss,locs):
        ...
        center_pos = np.array([bbox_gt[0]+(bbox_gt[2]-1)/2,bbox_gt[1]+(bbox_gt[3]-1)/2])
        size = np.array([bbox_gt[2], bbox_gt[3]])    
...
#        best_score = score[best_idx]
        ...
        best_idxs.append(Tensor(best_idx))
    gn_patches=torch.cat(gn_patches)
    pred_bboxs=torch.cat(pred_bboxs)
    best_idxs=torch.cat(best_idxs)
    return pred_bboxs

class _Block_A(Function):
    @staticmethod
    def forward(ctx,imgs,bboxs,clss,locs):
        pred_bboxs =fn(imgs,bboxs,clss,locs)
        return pred_bboxs
    @staticmethod
...
class MyBridge(torch.autograd.Function):
    ...
    def forward(ctx):
        anchors = generate_anchor(score_size=25) #score_size
#        anchor=anchors[idx,:]
        anchor=torch.from_numpy(anchors).cuda()
        anchor_num = len(cfg.ANCHOR.RATIOS) * len(cfg.ANCHOR.SCALES)
        hanning = np.hanning(25) #score_size
        window = np.outer(hanning, hanning)
        window = torch.from_numpy(np.tile(window.flatten(), anchor_num)).cuda().float()

#        locs2=locs.permute(1,2,3,0).contiguous().view(4,-1)
#        pred_bbox=locs2[:,idx]
#        pred_bbox=pred_bbox.reshape(4)
#        pred_bbox[0] = pred_bbox[0]*anchor1[2]+anchor1[0]
#        pred_bbox[1] = pred_bbox[1] * anchor1[3] + anchor1[1]
#        pred_bbox[2] = torch.exp(pred_bbox[2]) * anchor1[2]
#        pred_bbox[3] = torch.exp(pred_bbox[3]) * anchor1[3]
#        locs2.fill_(0)
#        locs2[:,idx]=1
#        ctx.save_for_backward(locs2,idx)
        return anchor,window
    @staticmethod
    def backward(ctx, grad_output):
#        locs2,idx, = ctx.saved_tensors
#        locs2[:,idx]=grad_output
#        grad_locs = locs2        
#        grad_locs=grad_locs.reshape(20,25,25,1).permute(3,0,1,2)
#        print(grad_output)
        return None
