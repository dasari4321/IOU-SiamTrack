...
#        val,idx=self.max_pool(cls[:,:,:,:,0])
        ...
        def _convert_score(clss):
            ...
            score2 = F.softmax(score, dim=2).data[:,:,1]
            return score2
...
#        best_idx = torch.argmax(pscore)
        ...
        lr = torch.mul(torch.mul(penalty,score),torch.tensor(cfg.TRACK.LR).cuda())

        cx=bboxs_new[:,0]+center_pos[0].repeat(bboxs_new.shape[2],1).permute(1,0)       
        cy=bboxs_new[:,1]+center_pos[1].repeat(bboxs_new.shape[2],1).permute(1,0)       
...
#
        ...
        with torch.no_grad():
            for i in range(batch):
                gt_iou[i,int(torch.argmax(IoU[i,:]))]=1.0
...
#        outputs2['gloss']     = gloss
        outputs2['iou_loss']  =  1.0-torch.mul(IoU,gt_iou).sum()
        return outputs2
#######################
