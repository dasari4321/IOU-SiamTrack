...
#        self.netG.wt=F.softmax(self.netG.wt, 0)
...
#        if ((outputs2['total_loss'] < 0.2) and (self.flag==False)):
#            self.flag=True
#        if(self.flag):
        iou_loss=self.IOU_loss(bboxs,bboxs_prev,clss,locs)        
#        else:
#            iou_loss=torch.ones(1).cuda().float()
...
#        # window penalty
        ...
        val,idx=self.max_pool(pscore.unsqueeze(0).permute(2,0,1))
...
#            
        lr=torch.mul(pscore,gt_score*self.netG.LR).sum(dim=0)                        
##        print(lr)
#        # smooth bbox
